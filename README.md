...
Rilis ini mengumpulkan **seluruh perkembangan setelah v1.2**.
 
**Fitur besar**
- **Cari Jebakan** — sadar-mode (Santai/Normal/Brutal) + indikator "lawan mentok
  X/Y" + filter sembunyikan imbuhan umum.
- **Analisa Match** — rekam (Alt+A/Alt+S) lalu lihat pola panjang sambungan per
  menit, sebaran, dan kaitan dengan panjang katamu.
- **Profil ganda** — beberapa progres terpisah dalam satu aplikasi.
- **Kamus cadangan diperluas besar-besaran** — puluhan ribu kata dari folder +
  sumber online yang menyegarkan diri.
- **Update otomatis** — unduh versi baru & pasang saat restart.
- **Discord Rich Presence** — status main tampil di Discord.
- **Mode Grafis (Lite / High)** di menu Kontrol.
**Tampilan & navigasi**
- Dialog **Tentang Aplikasi** baru + tautan Discord/Roblox/GitHub.
- **Sidebar** ditata ulang jadi hub Profil/Bantuan/Tentang/Informasi Update,
  bisa diciutkan, kategori membuka-menutup.
- **Tombol dukungan Ko-fi**.
- **REKOR HARIAN** menggantikan "Terbaik Hari Ini".
- Dialog **didesain ulang** (tanpa bingkai, ikut tema) + animasi loading saat login.
- Penyegaran font (kategori sidebar, Tentang, daftar kata).
**Kamus & kualitas data**
- **Penanda DiCoba** pada kata cadangan.
- **Pembersihan kamus otomatis** (kata pendek/bersimbol dibuang).
- Kata cadangan teratas muncul **di atas** saat awalan terindeks penuh.
- **Kontribusi kata** — kata yang kamu temukan ikut memperkaya kamus bersama.
**Perbaikan**
- Jumlah kata cadangan di `.exe` kini **cocok dengan sumber** (cache wordlist
  online disegarkan saat build).
- **Panduan cepat** tidak lagi terpotong di jendela kecil (tata letak menyesuaikan).
